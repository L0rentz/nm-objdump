...
int check_format_next_x86(char *data, int offset,
    size_t fd_size, errors_t *s_errors)
{
    Elf32_Ehdr *elf = (Elf32_Ehdr *)&data[offset];
...
int check_format_x86(char *data, int offset,
    size_t fd_size, errors_t *s_errors)
{
    if (fd_size < offset
    ...
    if (check_format_next_x86(data, offset, fd_size, s_errors))
        return (1);
    return (0);
}

void archive_offset_loop_x86(struct stat fd_stat,
    char *data, errors_t *s_errors)
{
    ...
    for (char obj_name[255], size[11], i = 0; offset < fd_stat.st_size; i++) {
        ...
        if (strlen(size) > 0 && i > 1) {
            ...
            if (check_format_x86(data, offset, fd_stat.st_size, s_errors))
                continue;
            printf("\n%s:", obj_name);
            objdump_x86(&data[offset], s_errors, 1);
        }
    }
}
...
void parse_archive_x86(errors_t *s_errors)
{
    ...
    archive_offset_loop_x86(fd_stat, data, s_errors);
